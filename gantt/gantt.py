...
__version__ = '0.7.0'
__last_modification__ = '2020-11-12'
...
class _my_svgwrite_drawing_wrapper(svgwrite.Drawing):
    ...
    def save(self, width='100%', height='100%'):
        ...
        if sys.version_info[0] == 2:
            test = isinstance(self.filename, types.FileType) or isinstance(self.filename, types.InstanceType)
...
def _flatten(thelist, ltypes=(list, tuple)):
    """
    ...
    i = 0
    while i < len(thelist):
        while isinstance(thelist[i], ltypes):
            if not thelist[i]:
                thelist.pop(i)
                i -= 1
                break
            else:
                thelist[i:i + 1] = thelist[i]
        i += 1
    return ltype(thelist)
...
class GroupOfResources(object):
    ...
    def __init__(self, name, fullname=None):
        ...
        """
        __LOG__.debug(f'** GroupOfResources::__init__(name = "{name}")')
        self.name = name
        self.vacations = []
    ...
    def nb_elements(self):
        ...
        """
        __LOG__.debug(f'** GroupOfResources::nb_elements (name: {self.name})')
    ...
    def is_available(self, date):
        ...
        if date in VACATIONS:
            __LOG__.debug(f'** GroupOfResources::is_available({self.name}, {date}) : False (global vacation)')
            return False
        ...
        for h in self.vacations:
            ...
            if date >= dfrom and date <= dto:
                __LOG__.debug(f'** GroupOfResources::is_available({self.name}, {date}) : False (group vacation)')
                return False

        # Test if at least one resource is avalaible
        for r in self.resources:
            if r.is_available(date):
                __LOG__.debug(f'** GroupOfResources::is_available({self.name}, {date}) : True {r.name}')
                return True

        __LOG__.debug(f'** GroupOfResources::is_available({self.name}, {date}) : False')
        return False
    ...
    def search_for_task_conflicts(self, all_tasks=False):
        ...
        for d in ke:
            ...
            if all_tasks:
                ...
                overcharged_days[d] = affected_days[d]
                __LOG__.warning(f'** GroupOfResources "{self.name}" has more than {self.nb_elements()} tasks on day {d}/{affected_days[d]}')
...
class Resource(object):
    ...
    def __init__(self, name, fullname=None):
        ...
        fullname -- long name given to the resource
        """
        __LOG__.debug(f'** Resource::__init__({name})')
        self.name = name
        if fullname is not None:
            self.fullname = fullname
        else:
    ...
    def is_available(self, date):
        ...
        # global VACATIONS
        if date in VACATIONS:
            __LOG__.debug(f'** Resource::is_available({self.name}, {date}) : False (global vacation)')
            return False
        ...
        for g in self.member_of_groups:
            for h in g.vacations:
                dfrom, dto = h
                if date >= dfrom and date <= dto:
                    __LOG__.debug(f'** Resource::is_available({self.name}, {date}) : False (Group {g.name})')
                    return False
        ...
        for h in self.vacations:
            dfrom, dto = h
            if date >= dfrom and date <= dto:
                __LOG__.debug(f'** Resource::is_available({self.name}, {date}): False')
                return False
        __LOG__.debug(f'** Resource::is_available({self.name}, {date}): True')
        return True
    ...
    def search_for_task_conflicts(self, all_tasks=False):
        ...
        for d in ke:
            if len(affected_days[d]) > 1:
                overcharged_days[d] = affected_days[d]
                __LOG__.warning(f'** Resource "{self.name}" has more than one task on day {d}/{affected_days[d]}')
...
class Task(object):
    ...
    def __init__(
        ...
        if nonecount != 1 and (self.duration is None or depends_of is None):
            __LOG__.error(f'** Task "{fullname}" must be defined by 2 of 3 limits (start, stop, duration) ({self.start}, {self.stop}, {self.duration})')
    ...
    def start_date(self):
        ...
        if self.start is not None:
            ...
            if self.depends_of is None:
                ...
                if prev_task_end > self.start:
                    __LOG__.warning(f'** Due to dependencies, Task "{self.fullname}", will not start on date {self.start} but {prev_task_end}')
        ...
        elif self.duration is not None and self.depends_of is not None and self.stop is None:
            prev_task_end = self.depends_of[0].end_date()
            for t in self.depends_of:
                if isinstance(t, Milestone):
                    if t.end_date() > prev_task_end:
        ...
        if self._cache_start_date != self.start:
            __LOG__.warning(f'** starting date for task "{self.fullname}" is changed from {self.start} to {self._cache_start_date}')
        return self._cache_start_date
    ...
    def svg(
        ...
        if not self.display:
            __LOG__.debug(f'** Task::svg ({self.name}) display off')
            return(None, 0)
        ...
        if scale == DRAW_WITH_DAILY_SCALE:
            ...
            def _time_diff(end_date, start_date):
                return dateutil.relativedelta.relativedelta(end_date, start_date).months \
                    + dateutil.relativedelta.relativedelta(end_date, start_date).years*12
    ...
    def svg_dependencies(self, prj):
        ...
        if self.depends_of is None:
            ...
            for t in self.depends_of:
                if isinstance(t, Milestone):
                    if None not in [t.drawn_x_end_coord, t.drawn_y_coord, self.drawn_x_begin_coord] \
...
class Milestone(Task):
    ...
    def end_date(self):
        ...
        creation or the one calculated after checking dependencies
        """
        __LOG__.debug(f'** Milestone::end_date({self.name})')
    ...
    def svg(
        ...
        offset -- X offset from image border to start of drawing zone
        """
        __LOG__.debug('** Milestone::svg ({0})'.format(
            {
                'name': self.name,
                'prev_y': prev_y,
                'start': start,
        ...
        if not self.display:
            __LOG__.debug(f'** Milestone::svg (name: {self.name}) display off')
            return (None, 0)
...
class Project(object):
    ...
    def _svg_calendar(
        ...
        for x in range(maxx):
            ...
            if scale == DRAW_WITH_DAILY_SCALE:
                # draw vacations
                if (start_date + datetime.timedelta(days=x)).weekday() in _not_worked_days() \
                        or (start_date + datetime.timedelta(days=x)) in VACATIONS:
                    vlines.add(svgwrite.shapes.Rect(
                ...
                if jour.isocalendar()[1] == 1 and jour.month == 1:
                    vlines.add(svgwrite.text.Text('{0}'.format(jour.year),
                                                  ...
                                                  fill='#400000', stroke='#400000', stroke_width=0,
                                                  font_family=_font_attributes()['font_family'],
                                                  font_size=15+5, font_weight="bold"))
                # Month name
                if jour.day <= 7:
                    vlines.add(svgwrite.text.Text(
                        jour.strftime("%B"),
                        insert=((x*10+1+offset)*mm, 10*mm),
                        fill='#800000', stroke='#800000', stroke_width=0,
    ...
    def make_svg_for_tasks(
        ...
        """
        if len(self.tasks) == 0:
            __LOG__.warning(f'** Empty project: {self.name}')
            return
        ...
        if start_date > end_date:
            __LOG__.critical(f'start date {start_date} > end_date {end_date}')
            sys.exit(1)
        ...
        if scale == DRAW_WITH_DAILY_SCALE:
            ...
            if dateutil.relativedelta.relativedelta(end_date, start_date).days == 0:
                maxx = dateutil.relativedelta.relativedelta(end_date, start_date).months \
                    + dateutil.relativedelta.relativedelta(end_date, start_date).years * 12
            else:
                maxx = dateutil.relativedelta.relativedelta(end_date, start_date).months \
                    + dateutil.relativedelta.relativedelta(end_date, start_date).years * 12 + 1
        elif scale == DRAW_WITH_QUATERLY_SCALE:
            # how many quarter do we need to draw ?
            __LOG__.critical('DRAW_WITH_QUATERLY_SCALE not implemented yet')
            sys.exit(1)
    ...
    def _svg_legend(self, x_start: int):
        ...
        if not self.legends:
            __LOG__.debug('** Project::_svg_legend - no legend - skipping generation of box')
            return Point(0, 0), drawing
    ...
    def make_svg_for_resources(
        ...
        for r in resources:
            ...
            ress.add(svgwrite.text.Text(
                r.fullname,
                insert=(3*mm, (nline*10+7)*mm),
                fill=_font_attributes()['fill'], stroke=_font_attributes()['stroke'],
                stroke_width=_font_attributes()['stroke_width'], font_family=_font_attributes()['font_family'],
                font_size=15+3))
            # ldwg.add(ress)
        ...
        if self.name != "":
            ...
            if ((self.start_date() >= start and self.end_date() <= end)
                ...
                fprj.add(
                    svgwrite.text.Text(
                        self.name,
                        insert=((6*level+3+offset)*mm, ((prev_y)*10+7)*mm),
                        fill=_font_attributes()['fill'],
                        stroke=_font_attributes()['stroke'],
                        stroke_width=_font_attributes()['stroke_width'],
                        font_family=_font_attributes()['font_family'],
    ...
    def is_in_project(self, task):
        ...
        task -- Task object
        """
        return any([t.is_in_project(task) for t in self.tasks])
    ...
    def csv(self, csv=None):
        ...
        if csv is not None:
            ...
            import io
            if sys.version_info[0] == 2:
                test = isinstance(csv, types.FileType) or isinstance(csv, types.InstanceType)
            elif sys.version_info[0] == 3:
